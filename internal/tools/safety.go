...
func (e ErrWriteDisabled) Error() string {
	...
	// переменной окружения, в сервисе — правами, выданными на экране
	// согласия. Не назвать оба варианта — значит отправить
	...
	return fmt.Sprintf(
		"Инструмент %s изменяет данные в магазине, а у этого подключения прав на запись нет.\n\n"+
			...
			"Если сервер подключён как сервис — при подключении право «Изменение»\n"+
			"не было выдано. Удалите коннектор в Claude, добавьте заново и отметьте\n"+
			"«Изменение» на экране согласия (для API-токена — выпустите токен с записью).\n\n"+
